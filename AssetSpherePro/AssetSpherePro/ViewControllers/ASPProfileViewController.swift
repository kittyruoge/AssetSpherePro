...
final class ASPProfileViewController: ASPBaseViewController,
    ...
    /// Called when the user signs out, so the app can return to the auth flow.
    var onSignOut: (() -> Void)?
    ...
    private func asp_reload() {
        ...
        // Sign out / exit guest mode. Available in every session so the user
        // always has a clear way to leave the current account.
        let isGuest = ASPUserManager.shared.isGuest
        let signOutButton = ASPPrimaryButton(title: isGuest ? "Exit Guest Mode" : "Sign Out", style: .glass)
        signOutButton.addTarget(self, action: #selector(asp_signOutTapped), for: .touchUpInside)
        ...
        if !ASPUserManager.shared.isGuest {
            ...
            contentStack.setCustomSpacing(12, after: signOutButton)
